...
printmaxinval[findmaximum_] := (
	If[Not[StringQ[findmaximum]],
		(WriteString[$stream,InputForm[findmaximum[[1]]]];
		WriteString[$stream,","];
		WriteString[$stream,InputForm[Values[findmaximum[[2]][[1]]]]];
		WriteString[$stream,","];
		WriteString[$stream,InputForm[Values[findmaximum[[2]][[2]]]]];),
...
printfindmax[distanceres_,cons_,vars_] := Module[
	{},
	WriteString[$stream,InputForm[distanceres]];
	WriteString[$stream,","];
	WriteString[$stream,InputForm[Check[islinear[distanceres],"error"]]];	
	WriteString[$stream,","];
...
pks[p_,q_,cons_,vars_] := Module[
	...
	WriteString[$stream,InputForm[disres]];
	WriteString[$stream,","];
	If[Not[StringQ[disres]],
		(disresr2[r2_] = (disres /. r1->r2);
		WriteString[$stream,islinear2[disresr2]];
		WriteString[$stream,","];
...
ptvd[p_,q_,cons_,vars_,varsminmax_] := Module[
	...
	WriteString[$stream,InputForm[tvdres]];
	WriteString[$stream,","];
	If[Not[StringQ[tvdres]],(
		WriteString[$stream,islinear[tvdres]];	
		WriteString[$stream,","];
...
pkl[p_,q_,cons_,vars_,varsminmax_] := Module[
	...
	WriteString[$stream,InputForm[klres]];
	WriteString[$stream,","];
	If[Not[StringQ[klres]],(
		WriteString[$stream,islinear[klres]];	
		WriteString[$stream,","];
